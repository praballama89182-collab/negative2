...
def load_bulk_file(bulk_file_path):
    ...
    # Mapping your specific 7-day attribution headers to standard names [cite: 122, 126]
    column_mapping = {
    ...
    # Dynamic detection for your specific sheet names [cite: 134]
...
def aggregate_data(sp_df, sb_df):
    """Standardize data and combine SP and SB reports."""
    ...
    # Round metrics to 1 decimal place [cite: 148]
    for col in ['Spend', 'Sales', 'CPC']:
        df[col] = df[col].round(1)
    return df

def get_exact_keyword_analysis(df):
    """Returns exact search terms sorted by spend with percentage ACOS."""
    exact_df = df.copy()
...
def perform_ngram_analysis(df, n):
    """Breaks down search terms into n-grams while keeping campaign context[cite: 134]."""
    res = []
